...
from daisytuner.profiling.metrics.arm64fx import Runtime
...
def test_copy():
    ...
    metric = Runtime(sdfg, hostname="ault28.cscs.ch")
    # args = {
    ...
    assert np.allclose(value, 0.002, atol=0.0, rtol=1.3)
...
def test_triad():
    ...
    assert metric.has_values()
    value = metric.compute()
    assert np.allclose(value, 0.00008, atol=0.0, rtol=1.3)
