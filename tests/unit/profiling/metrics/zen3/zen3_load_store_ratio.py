...
from daisytuner.profiling.metrics.zen3 import LoadStoreRatio
...
def test_copy():
    ...
    metric = LoadStoreRatio(sdfg, hostname="lukas-Nitro")
    # args = {
...
def test_triad():
    ...
    assert metric.has_values()
    value = metric.compute()
    assert np.allclose(value, 2.0, atol=0.2, rtol=0.0)
