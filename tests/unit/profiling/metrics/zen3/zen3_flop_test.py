...
from daisytuner.profiling.metrics.zen3 import FLOP
...
def test_copy():
    ...
    metric = FLOP(sdfg, hostname="lukas-Nitro")
    # args = {
    ...
    assert np.allclose(value, 10e-6, atol=1e-6, rtol=0.0)
...
def test_triad():
    ...
    assert metric.has_values()
    value = metric.compute()
    assert np.allclose(value, (32768 * 2 + 10) * 1e-6, atol=1e-6, rtol=0.0)
