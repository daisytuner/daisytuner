...
from daisytuner.profiling.metrics.zen import Runtime
...
def test_copy():
    ...
    metric = Runtime(sdfg, hostname="lukas-TUF-Gaming-FX505DV-FX505DV")
    # args = {
    ...
    assert np.allclose(value, 0.002, atol=0.001, rtol=0.0)
...
def test_triad():
    ...
    assert metric.has_values()
    value = metric.compute()
    assert np.allclose(value, 0.0002, atol=0.0001, rtol=0.0)
