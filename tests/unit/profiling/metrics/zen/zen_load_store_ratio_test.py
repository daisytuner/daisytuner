...
from daisytuner.profiling.metrics.zen import LoadStoreRatio
...
def test_copy():
    ...
    metric = LoadStoreRatio(sdfg, hostname="lukas-TUF-Gaming-FX505DV-FX505DV")
    # args = {
    ...
    assert np.allclose(value, 0.06, atol=0.01, rtol=0.0)
...
def test_triad():
    ...
    assert metric.has_values()
    value = metric.compute()
    assert np.allclose(value, 0.12, atol=0.01, rtol=0.0)
