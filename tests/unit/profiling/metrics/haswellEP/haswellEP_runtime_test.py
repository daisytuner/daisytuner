...
from daisytuner.profiling.metrics.haswellEP import Runtime
...
def test_copy():
    ...
    metric = Runtime(sdfg, hostname="garbenheim")
    # args = {
    ...
    assert np.allclose(value, 0.006, atol=0.0, rtol=1.3)

    values_per_thread = metric.compute_per_thread()
    assert values_per_thread.shape == (24,)
...
def test_triad():
    ...
    assert metric.has_values()
    value = metric.compute()
    assert np.allclose(value, 0.007, atol=0.0, rtol=1.3)
