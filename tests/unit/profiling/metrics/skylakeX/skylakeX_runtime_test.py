...
from daisytuner.profiling.metrics.skylakeX import Runtime
...
def test_copy():
    ...
    metric = Runtime(sdfg, hostname="ault01.cscs.ch")
    # args = {
    ...
    assert np.allclose(value, 0.003, atol=0.0001, rtol=0.0)
...
def test_triad():
    ...
    assert metric.has_values()
    value = metric.compute()
    assert np.allclose(value, 0.0001, atol=0.0001, rtol=0.0)
