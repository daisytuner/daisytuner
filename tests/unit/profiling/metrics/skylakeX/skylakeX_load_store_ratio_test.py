...
from daisytuner.profiling.metrics.skylakeX import LoadStoreRatio
...
def test_copy():
    ...
    metric = LoadStoreRatio(sdfg, hostname="ault01.cscs.ch")
    # args = {
...
def test_triad():
    ...
    assert metric.has_values()
    value = metric.compute()
    assert np.allclose(value, 2.0, atol=0.02, rtol=0.0)
