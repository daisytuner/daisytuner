...
from daisytuner.profiling.metrics.broadwellEP import LoadStoreRatio
...
def test_copy():
    ...
    metric = LoadStoreRatio(sdfg, hostname="ault21.cscs.ch")
    # args = {
    ...
    assert np.allclose(value, 1.0, atol=0.01, rtol=0.0)
...
def test_triad():
    ...
    assert metric.has_values()
    value = metric.compute()
    assert np.allclose(value, 2.0, atol=0.01, rtol=0.0)
