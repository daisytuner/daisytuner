...
from daisytuner.profiling.metrics.broadwellEP import FLOP
...
def test_copy():
    ...
    metric = FLOP(sdfg, hostname="ault21.cscs.ch")
    # args = {
    ...
    assert np.allclose(value, 0.0, atol=4e-5, rtol=0.0)
...
def test_triad():
    ...
    assert metric.has_values()
    value = metric.compute()
    assert np.allclose(value, (32768 * 2) * 1e-6, atol=4e-5, rtol=0.0)
